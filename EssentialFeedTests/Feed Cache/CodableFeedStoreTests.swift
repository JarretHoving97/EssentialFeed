...
final class CodableFeedStoreTestss: XCTestCase, FailableFeedStore {
    ...
    func test_storeSideEffects_runSerially() {
        let sut = makeSUT()
        var completedOperartionsInOrder = [XCTestExpectation]()
        
        let op1 = expectation(description: "Operation 1")
        
        sut.insert(uniqueImageFeed().local, timestamp: Date()) { _ in
            completedOperartionsInOrder.append(op1)
            op1.fulfill()
        }
        ...
        sut.insert(uniqueImageFeed().local, timestamp: Date()) { _ in
            completedOperartionsInOrder.append(op2)
            op2.fulfill()
        }
        ...
        sut.insert(uniqueImageFeed().local, timestamp: Date()) { _ in
            completedOperartionsInOrder.append(op3)
            op3.fulfill()
        }
        
        waitForExpectations(timeout: 5.0)
        
        XCTAssertEqual(completedOperartionsInOrder, [op1, op2, op3])
    }
    ...
    private func deleteCache(from sut: FeedStore) -> Error? {
        ...
        sut.deletedCachedFeed { receivedDeletionError in
    ...
    private func expect(_ sut: FeedStore, toRetrieve expectedResult: RetrievedCachedFeedResult, file: StaticString = #file, line: UInt = #line) {
        ...
        sut.retrieve { retrievedResult in
            ...
            default:
                XCTFail("Extected to retrieve \(expectedResult), got \(retrievedResult) instead", file: file, line: line)
            }
