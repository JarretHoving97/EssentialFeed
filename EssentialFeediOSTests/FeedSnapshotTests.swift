...
class FeedSnapshotTests: XCTestCase {
    ...
    func assert(snapshot: UIImage, named name: String, file: StaticString = #file, line: UInt = #line) {
        ...
        guard let storedSnapshot = try? Data(contentsOf: snapshotURL) else {
            XCTFail("Failed load stored snapshot at URL: \(snapshotURL), Use the `record method to store a snapshot before testing`")
            return
        }
        
        if snapshotData != storedSnapshot {
            let temporarySnapShotURL = URL(fileURLWithPath: snapshotURL.lastPathComponent)
            
            try? snapshotData?.write(to: temporarySnapShotURL)
            
            XCTFail("News snapshot does not match stored snapshot, New snapshot URL: \(temporarySnapShotURL), Stored snapshot URL \(snapshotURL)", file: file, line: line)
        }
    }
